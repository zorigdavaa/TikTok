    ...
    void Update()
    {
        if (IsPlaying)
        {
            if (ControlAble)
            {
                ...
                if (IsClick && transform.position.x >= -4.80f && transform.position.x <= 4.80f && !shouldROt)
                {
                ...
                if (transform.position.x < -4.80f)
                {
                    transform.position = transform.position.ChangeX(-4.78f);
                }
